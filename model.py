...
class M2S_ADD(nn.Module):
    def __init__(self, d_args, device):
        ...
        self.conv_time=CONV(device=self.device,
			out_channels = d_args['out_channels'],  # 70
			kernel_size = d_args['first_conv'],  # 128 
            in_channels = d_args['in_channels'] # 1
        )
        
        self.first_bn = nn.BatchNorm2d(num_features = 1, track_running_stats = False)
        
        self.selu = nn.SELU(inplace=True)
        
        # Note that here you can also use only one encoder to reduce the network parameters which is jsut half of the 0.44M (mentioned in the paper). 
        # I was doing some subband analysis and forget to remove the use of two encoders.  I also checked with one encoder and found same results. 
    ...
    def forward(self, x, Freq_aug=False):
        data_1 = x.swapaxes(1,2) 
        d = torch.split(data_1, 1, dim=1)  
        l = d[0] 
        r = d[1]  
        l = l.swapaxes(1,2)  
        r = r.swapaxes(1,2)  
        data_left = torch.squeeze(l) 
        data_right = torch.squeeze(r) 

        # Left side
        #follow sincNet recipe 
        nb_samp = data_left.shape[0] 
        len_seq = data_left.shape[1] 
        x_l = data_left.view(nb_samp,1,len_seq) 
        # Freq masking during training only
        if (Freq_aug==True):
            x_l=self.conv_time(x_l,mask=True)  
        else:
            x_l=self.conv_time(x_l,mask=False)
        """
        ...
        """
        x_l = x_l.unsqueeze(dim=1)  
        x_l = F.max_pool2d(torch.abs(x_l), (3,3))          
        x_l = self.first_bn(x_l)
        x_l = self.selu(x_l)
        # encoder structure for spectral GAT 【ResBlock】
        e1=self.encoder1(x_l)           
        # Spectral_attention
        # max-pooling along time with absolute value  (Attention in spectral part)
        x_left_max,_=torch.max(torch.abs(e1),dim=3)
        x_left_gat1=self.GAT_layer1(x_left_max.transpose(1,2))  
        x_left_pool1=self.pool1(x_left_gat1)
        out1=self.proj1(x_left_pool1.transpose(1,3))
        out1=out1.view(out1.shape[0],out1.shape[1],out1.shape[3]) 
    

        # right side
        nb_samp = data_right.shape[0] 
        len_seq = data_right.shape[1] 
        x_r = data_right.view(nb_samp,1,len_seq) # reshape
        # Freq masking during training only
        if (Freq_aug==True):
            x_r=self.conv_time(x_r,mask=True)  
        else:
            x_r=self.conv_time(x_r,mask=False)
        """
        Different with the our RawNet2 model, we interpret the output of sinc-convolution layer as 2-dimensional image with one channel (like 2-D representation).
        """
        x_r = x_r.unsqueeze(dim=1)  
        x_r = F.max_pool2d(torch.abs(x_r), (3,3)) 
        x_r = self.first_bn(x_r)
        x_r = self.selu(x_r)
        e2=self.encoder1(x_r)  
